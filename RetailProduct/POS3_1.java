...
public class POS3_1 extends getResultSet implements Runnable {
	...
	public DefaultTableModel promomodel = new DefaultTableModel() {
		...
		public boolean isCellEditable(int row, int column) {
			return column == 0 || column == 1 || column == 2 || column == 3|| column == 4 
					|| column == 5 || column == 6 
					|| column == 7
					|| column == 8 || column == 9 || column == 10 ? false : true;
		}
	};
...
//	prst.setString(56, invdate.getText()); //orderdate
	...
	private String Col[] = { "X", "Item", "Name", "Qty", "Rate", "MRP", "Gross", 
	...
	public DefaultTableModel model = new DefaultTableModel() {
		...
		public boolean isCellEditable(int row, int column) {
			...
			try {
				...
				switch (allow) {
				case "Y":
					return column == 0 || column == 13 || column == 3 || column == 4 || column == 7 || column == 12 || column == 14
							|| column == 15 ? true : false;
				case "N":
					return column == 0 || column == 0 || column == 3 || column == 7 || column == 12 || column == 14 || column == 15
							? true : false;
				}
	...
	public POS3_1() throws Throwable {
		// this.closeconn();
		// setStandardData();
		// getPOSconfigValues();
		initialize();
	...
	private void initialize() throws Throwable {
		POSFrame3_1 = new JFrame();
		POSFrame3_1.setResizable(false);
		POSFrame3_1.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		POSFrame3_1.getContentPane().setFont(new Font("Times New Roman", Font.PLAIN, 13));
		POSFrame3_1.getContentPane().setBackground(Color.WHITE);
		POSFrame3_1.getContentPane().setLayout(null);
		POSFrame3_1.setTitle("Business Application - Stand Alone Store Edition");
		POSFrame3_1.setOpacity(1.0f);
		POSFrame3_1.setBackground(Color.DARK_GRAY);
		POSFrame3_1.setAlwaysOnTop(true);
		POSFrame3_1.setBounds(1, 0, 1356, 772);
		POSFrame3_1.getContentPane().setEnabled(true);
		POSFrame3_1.getContentPane().setBackground(Color.CYAN);
		POSFrame3_1.getContentPane().setLayout(null);
		...
		POSFrame3_1.getContentPane().add(HeaderPanel);
		...
		POSFrame3_1.getContentPane().add(possplit);
		...
		panel_4.setBorder(new TitledBorder(new BevelBorder(BevelBorder.LOWERED, new Color(64, 64, 64), new Color(255, 255, 255), new Color(255, 0, 255), new Color(255, 255, 0)), "Select  Order Type and Table Here", TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 51)));
		...
		comboOrderType
				.setModel(new DefaultComboBoxModel(new String[] {"N/A", "TABLE", "HOME DELIVERY", "COUNTER", "SWIGGY", "ZOMATO"}));
		...
		// jun25
		POSFrame3_1.getContentPane().add(pmt);
		...
		lblNewLabel_25 = new JLabel("POS3_1");
		lblNewLabel_25.setBounds(24, 163, 56, 16);
		...
		POSFrame3_1.getContentPane().add(panelscan);
		...
		POSFrame3_1.getContentPane().add(Site_code);
		...
		POSFrame3_1.getContentPane().add(btn_Payment);
		...
		POSFrame3_1.getContentPane().add(btn_Save);
		...
		POSFrame3_1.getContentPane().add(btn_print);
	...
	public void decorate() throws Throwable {
		...
		switch (vpara714) {
		case "N":
			POSFrame3_1.setUndecorated(false);
			break;
		case "Y":
			POSFrame3_1.setUndecorated(false);
			break;
		...
		POSFrame3_1.getContentPane().add(txtpnFViewOrder);

		billtimePanel = new JPanel();
		billtimePanel.setBounds(1027, 4, 323, 27);
		POSFrame3_1.getContentPane().add(billtimePanel);
		...
		POSFrame3_1.getContentPane().add(dispPanel);
	...
	private void setPaymentPanel() {
		...
		POSFrame3_1.getContentPane().add(pmtPanel);
	...
	public String showTableData(String value, DefaultTableModel model) throws Throwable {
		...
		if (neginvallow == "Y") {
			...
			if (action == "Insert") {
				...
				//new General().msg("Apply diff price" +"" +vapplydiffPrice);
 				switch(vapplydiffPrice)   //If Config Setiing is not not Allow diff Price
	 			{
	 					case "N" :
  	 				    vtype = "RESTOBAR";
	 			 ...
	 			 switch(vapplydiffPrice)    //If Diffrent Price Apply then Only 
	 			 {
	 			    	 ...
	 			    	 if (vordertype.equals("HOME DELIVERY"))
	 			    	 {
	 			    	    ...
	 			    	    switch(vapplyBasePriceForTelephone.trim())
	 						{
	 								case "N" :  //Apply diffrent price
	 				 			        //new General().msg("I am in Count Diff Price Option  N");
   ...
   ////REsto Diff price apply 
				...
				if (rs.next()) {
					...
					if (varticlereturn > 0) {
						...
						//{ "X", "Item", "Name", "Qty", "Rate", "MRP", "Gross", "Disc Item", "Bill Disc", "Disc",
						//"Tax", "Amount", "Tax%", "Batch(Y/N)", "BatchNo", "ExpDate", "Free(Qty)/Disc", "Order", "Table", "KOT" };
 					  	//vaddSub 
	...
	public String showEditTableData(String SQL, DefaultTableModel model) throws Throwable {
		...
		while (rs.next()) {
			...
			model.insertRow(0, new Object[] { new Boolean(false), rs.getString(1), rs.getString(2), rs.getString(3),
							rs.getString(4),  rs.getString(5),   rs.getString(6), 
...
//				                  "OrderType", "Table", "KOT" };
	...
	public void MyCustSearch(String sqlsearch, String[] Col, int colno , String vselecttype) throws SQLException, ClassNotFoundException {
	...
	private void showTableHoldData() throws ClassNotFoundException, SQLException {
		...
		//promotable.removeAll();
 		new gnTable().RemoveTableRows(promomodel);
		String col[] = { "Type", "OrderType","KOT", "TableNo", "OrderNo" ,  "Items", "Amount", "Mobile", "Customer", "StartOn",
				"Now" };
		String vtype = "Order";
	...
	private void showHoldBill(String vordertype, String vkotno, String vtableno, int vitemcount, String vmobile , String vorderno)
		...
		{
  		String SQL = "";
		String Col[] = { "X", "Item", "Name", "Qty", "Rate", "MRP", "Gross", "Disc Item", "Bill Disc", "Disc", "Tax",
				"Amount", "Tax%", "Taste", "BatchNo", "ExpDate", "Free(Qty)/Disc", "Order", "Table", "KOT" };
		model.setColumnIdentifiers(Col);
		if (vordertype.equals("TABLE")) {
		...
		if (!vordertype.equals("TABLE")) {
			SQL = "select a.Article_code , b.name , a.qty , a.Sales_price , a.mrp , a.Gross_Amount , "
					...
					+ " from  tmp_pos_sales_detail_resto  a , msArticle_master b  " + " where a.Company_code = " + vcomp
					+ "  " + " and a.Site_code = " + vstore + "  " + " and a.company_code  = b.company_code "
					+ " and   orderno = '"+vorderno+"'  and ordertype = '" + vordertype + "'  "
					+ " and a.article_code = b.Article_code     ";
 		}
 		rs = this.getSPResult(SQL);
		while (rs.next()) {
	...
	private void displayList() throws ClassNotFoundException, SQLException {
		...
		if (!vordertype.equals("TABLE")) {
			SQL = "select   a.OrderType , a.TableNo , a.KOT , a.Article_code , b.name , a.qty , a.Sales_price,"
					...
					+ " from  tmp_pos_sales_detail_resto  a , msArticle_master b  " + " where a.Company_code = " + vcomp
					+ "  " + " and a.Site_code = " + vstore + "  " + " and a.company_code  = b.company_code "
					+ "  and  EAN = " + vmobile + "    and  orderno = " + vorderno + "    and ordertype = '" + vordertype + "'  "
					+ " and a.article_code = b.Article_code     ";
		}
...
private void printcancelOrder(String vordertype,   String vkotno,   String vtableno,   String vmobile , String vorderno) throws Throwable
{
 	//String vordertype = comboOrderType.getSelectedItem().toString().trim();
	//String vmobile =  mobile.getText() ;
	//String vtableno  = comboTable.getSelectedItem().toString().trim();
	...
	if (vstatus == "Sucess")
	{
		  ...
		  vmergetableno2  = mergetable3.getText();
			...
			String SQL = "Update tmp_pos_sales_detail_resto set tableno =  "+ vmergeinto+"  "
					...
					+ "  and ordertype = '"+vorderType+"' and tableno =  "+vmergetableno1+" " ;
 			
			String SQL1 = "Update tmp_pos_sales_detail_resto set tableno =  "+ vmergeinto+" "
			+ "  where company_code = "+vcomp+" and  site_code = "+vstore+" "
			+ "  and ordertype = '"+vorderType+"' and tableno =  "+vmergetableno2+" " ;
